...
def fill_last_values_of_colum_with_previos_value(df, colum_name):
    try:
        ...
        if 'Date' not in df.columns:
            print("In the entered data frame there is neither Date nor ticker, it is not possible to make the correct order, the default order is used.    Shape: " + str(df.shape))
...
def thread_list_is_alive(list_thread,producer,consumer ):
    ...
    for thr in  list_thread:
        if thr.is_alive():
            ...
            print(" The thread is not alive, proceed to restart.  Name: " + str(thr.name))
